...
function generatePage(name) {
    ...
    xhr.open("GET", "http://omega.maretec.org/api/?barragem="+params["barragem"], true);
    xhr.setRequestHeader('Content-Type', 'text/json');
    xhr.responseType = 'json';
    xhr.onload = function() {
      let flag = true;
      for(let i = 0; i < xhr.response.Barragens.length; i++) {
        if(xhr.response.Barragens[i].Folder == params["barragem"]) {
          for(let k = 0; k < xhr.response.Indicadores.length; k++) {
            ...
            google.charts.setOnLoadCallback(function() {generateMap(xhr.response.Indicadores[k],xhr.response.Barragens[i].Folder)});
            flag = false;
          }
...
function generateMap(elm,barragem) {
    console.log("elm.Folder = " + elm.Folder + "barragem =" + barragem);
    var jsonData = $.ajax({
      url: "http://omega.maretec.org/api/?chart="+elm.Folder+"&b="+barragem,
      dataType: "json",
      async: false
      }).responseText;
