...
function getAllUrlParams(url) {
    ...
    if (queryString) {
        ...
        if (paramName.match(/\[(\d+)?\]$/)) {
            ...
            obj[key].push(parseFloat(paramValue));
            }
        } else {
...
function generatePage(name) {
    ...
    xhr.open("GET", "http://omega.maretec.org/api/?caudal="+name, true);
    xhr.setRequestHeader('Content-Type', 'text/json');
    xhr.responseType = 'json';
    xhr.onload = function() {
      let flag = true;
      for(let i = 0; i < xhr.response.Caudais.length; i++) {
        if(xhr.response.Caudais[i].Folder == params["caudal"]) {
          for(let k = 0; k < xhr.response.Indicadores.length; k++) {
            document.getElementById("name").innerHTML = xhr.response.Caudais[i].Nome.charAt(0).toUpperCase() + xhr.response.Caudais[i].Nome.slice(1);
            google.charts.setOnLoadCallback(function() {generateMap(xhr.response.Indicadores[k],xhr.response.Caudais[i].Folder)});
            flag = false;
          }
...
function generateMap(elm, caudal) {
    console.log(elm.Folder + caudal);
    var jsonData = $.ajax({
      url: "http://omega.maretec.org/api/?chart="+elm.Folder+"&c="+caudal,
      dataType: "json",
      async: false
      }).responseText;
      var data = [["Date"]];
