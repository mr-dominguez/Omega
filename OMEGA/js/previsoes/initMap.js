...
function getJSONLocations(){
    let xhr = new XMLHttpRequest();
    xhr.open("GET", "http://omega.maretec.org/api/?file=LocationsForecast", true);
    xhr.setRequestHeader('Content-Type', 'text/json');
    xhr.responseType = 'json';
    xhr.onload = function() {
...
function putMarkers(Barragens) {
    for(var i = 0; i < Barragens.Barragens.length; i++){
        ...
        var circle = L.circle([barragem.Latitude, barragem.Longitude], {
            ...
            fillOpacity: 0.5,
            radius: 4000,
            url: "http://omega.maretec.org/html/previsoes/barragem.html?barragemPrevista="+barragem.Nome,
            info: barragem
        }).addTo(mymap);
